...
import {
  SupportedCurrenciesVersion,
  SupportedCurrencyServiceAbstract,
} from '#domain/abstracts/supported-currency-service/supported-currency-service';
...
const filterSupportedRates = (rate: Record<string, number>): Record<string, number> => {
  return Object.fromEntries(
    Object.entries(rate).filter(([currencyCode]) =>
      SUPPORTED_CURRENCY_CODES.includes(currencyCode as ICurrency['code']),
    ),
  );
};
...
export class SupportedCurrencyService implements SupportedCurrencyServiceAbstract {
  ...
  readonly findVersionByDate = async (
    date: ICurrencyRate['date'],
    trx?: ITransactionWithLock,
